...
ML_MODEL_PATH = "models/finger_mlp_model.pth"
...
def predict_fingers_by_model(hand_landmarks, model, device):
...
def main():
    ...
    finger_model = FingerMLP().to(device)
    finger_model.load_state_dict(torch.load(ML_MODEL_PATH, map_location=device))
    finger_model.eval()
    ...
    while True:
        ...
        if hands.hand_landmarks: #手が検出された場合
            ...
            for index, hand_landmarks in enumerate(hands.hand_landmarks): #検出された手を1つずつ取り出す(今回の場合，一人なので最大2つ)
                ...
                finger_cnt = predict_fingers_by_model(
                    hand_landmarks,
                    finger_model,
                    device
                )
        ...
        if key == ord("q"):
            break
        elif key == ord("c") and not is_locked:
            quiz_mode = True
            is_locked = True
        ...
        if quiz_mode:
            ...
            if quiz_waiting_cool_down:
                cv2.putText(frame, "Correct!")

                if now - last_correct_time >= COOL_DOWN_TIME:
                    quiz_waiting_cool_down = False
